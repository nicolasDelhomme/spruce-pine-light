#' ---
#' title: "Pine light (2024 genome) Biological QA"
#' subtitle: "spruce-pine-light"
...
#' ```
tx2gene <- suppressMessages(read_delim(here("analysis/pine/nfcoreResult/salmon/tx2gene.tsv"),delim="\t",
                                 col_names=c("TXID","GENE"))) %>%
  select(-X3)

#' * Raw data
filelist <- data.frame(Filenames = list.files(here("analysis/pine/nfcoreResult/salmon"), 
                          recursive = TRUE, 
                          pattern = "quant.sf",
                          full.names = TRUE)) %>%
...
write_tsv(samples,here("doc/samples_full_rank_pine.txt"))
...
#' `r emoji("point_right")` **The raw sequencing depth of replicate 4 (B-P67-4 and D-P56-4) are significantly lower than other replicates.**
#' 
...
#' `r emoji("point_right")` **Replicate 4 has different sequencing depth characteristics from other replicates.**
#' 
...
save(dds,file=here("analysis/pine/dds.rda"))
...
#' `r emoji("point_right")` **The libraries' size factors of replicate 4 are significantly lower than other replicates.**
#' 
...
boxplot(split(t(normalizationFactors(dds[,!dds$Replicate == 4])),dds[,!dds$Replicate == 4]$EcotypeLatitude),las=2,
        main="Sequencing libraries size factor by Ecotype in replicate 1-3",
        outline=FALSE)

boxplot(split(t(normalizationFactors(dds[,!dds$Replicate == 4])),dds[,!dds$Replicate == 4]$LightCondition),las=2,
        main="Sequencing libraries size factor by Light condition in replicate 1-3",
        outline=FALSE)

#' `r emoji("point_right")` **The scaling factor distribution is dependent of replicate (only 4), but not dependent of other variables.**
...
#' `r emoji("point_right")` **The scaling factor appear linearly proportional to the sequencing depth in all replicates.**
#' 
...
#' `r emoji("point_right")` **PC1 (37%) explains light condition. PC2 (11%) explains the outliers of each light condition group. The outliers are from totally different ecotypes, light conditions, and replicates.**
#' 
#' * PC1 vs PC3
...
#' `r emoji("point_right")` **PC1 (37%) explains light condition. PC2 (11%) and PC3 (6%) explain the outliers of each light condition group. The outliers are from totally different ecotypes, light conditions, and replicates.**
#' 
#' ### Pairs plot
...
#' `r emoji("point_right")` **PC1 and PC4 explain light conditions. PC5 explains ecotypes.**
#' 
#' ### Samples Distance
...
hm <- pheatmap(sampleDistMatrix,
         ...
         col=pal)
hm
#' `r emoji("point_right")` **B clusters together well. A and D are also well separated except for a pair of A and D samples, which are the ones driving PC2, that look different.**
#' 
#' ## Sequencing depth
...
#' `r emoji("point_right")` **For D, number of genes are quite varied, and seems to be affected by ecotype. The number of genes expressed in other light conditions are not affected by ecotype.**
#' 
#' ## Heatmap
#' 
...
#' `r emoji("point_right")` **Here a cutoff of `r vst.cutoff` is applied**
...
#' `r emoji("point_right")` **By selecting only informative genes, the expression seems to group well by light conditions where D is separated. A and B are close to each other but still remain as separate clades. We can still see the outliers of A and D here.**
#'
...
#' `r emoji("point_right")` **By selecting only informative genes, samples are grouped by light conditions where D is separated. A and B are close to each other but still remain as separate clades.**
#' 
