#' ---
#' title: "Sonali's Norway Spruce light experiment Biological QA - All samples"
...
#' Create a palette
pal = c("#00FFFF","#0000FF","#8A2BE2","#A52A2A","#DEB887","#5F9EA0","#7FFF00","#FF7F50","#DC143C","#00008B","#006400","#8B008B","#8B0000","#E9967A","#FFD700","#DAA520","#FFB6C1","#FFFF00")
...
samples <- read.csv("~/Git/UPSCb/projects/spruce-pine-light/doc/spruce.csv")

#' Read the HTSeq files in a matrix
#' res <- mclapply(dir("htseq-pine",pattern="*.txt",full.names=TRUE),function(fil){
res <- mclapply(dir("htseq-spruce",pattern="*.txt",full.names=TRUE),function(fil){
  read.delim(fil,header=FALSE,stringsAsFactors=FALSE)
},mc.cores=max(mcaffinity()))

#'names(res) <- sub(".*_P3462","P3462",sub("_sortmerna.*\\.txt","",dir("htseq",pattern="*.txt")))
#'names(res) <- sub(".*_P","P",sub("_sortmerna.*\\.txt","",dir("htseq-pine",pattern="*.txt")))
names(res) <- sub(".*_P","P",sub("_sortmerna.*\\.txt","",dir("htseq-spruce",pattern="*.txt")))
...
scatterplot3d(pc$x[,1],
              ...
              color=c(1,2,pal)[as.integer(samples$SampleDescription)],
              pch=19)
legend("topleft",pch=19,
       col=c(1,2,pal)[as.integer(samples$SampleDescription)],
       legend=levels(samples$SampleDescription),cex=0.7)
par(mar=mar)

#' Then the first two dimensions
#' The first dimension separates the light conditions (Dark, A & B). The second dimension shows  
#' variability within the two ecotypes
plot(pc$x[,1],
     pc$x[,2],
     xlab=paste("Comp. 1 (",percent[1],"%)",sep=""),
     ylab=paste("Comp. 2 (",percent[2],"%)",sep=""),
...
plot(pc$x[,2],
     ...
     col=c(1,2,pal)[as.integer(factor(samples$SampleDescription))],
     pch=19,
     main="Principal Component Analysis",sub="variance stabilized counts")
legend("topleft",pch=19,
       col=c(1,2,pal)[as.integer(samples$SampleDescription)],
...
#' The distinction between Dark and Light treated (A & B) is very clear in these 1000 first
#' genes.
...
#' the heatmap identifies no samples as possible outliers. Moreover, it is clear
